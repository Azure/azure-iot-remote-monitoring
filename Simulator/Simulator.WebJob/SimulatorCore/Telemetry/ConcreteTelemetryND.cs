...
namespace Microsoft.Azure.Devices.Applications.RemoteMonitoring.Simulator.WebJob.SimulatorCore.Telemetry
{
    ...
    /// directly.
    /// </summary>
    public class ConcreteTelemetryND<T> : ITelemetryND<T>
    {
        private readonly ILogger _logger;

        public ConcreteTelemetryND(ILogger logger)
        {
            _logger = logger;
        }
        ...
        public async Task SendEventsAsync(CancellationToken token, Func<T, Task> sendMessageAsync)
        {
            var groupCount = 0;
